...
import { requireAdmin, setAdminCors, isAdminUser } from "../lib/admin-auth.js";
import {
  appendRecentLog,
  computeLogStatus,
  getAutoStatus,
...
function isStaticAdminUser(telegramId) {
...
async function getRecentLogsForAdminStats(firebaseDbUrl, daysBack = 7, limit = 50) {
  try {
    const logs = [];
    const now = new Date();
...
async function getUserTelegramIdSimple(firebaseDbUrl, userId) {
  try {
    const response = await fetch(
...
app.all("/api/daily-broadcast", async (req, res) => {
  ...
  const validSecret = expectedSecret && providedAuth === `******;
...
app.all("/api/admin-stats", async (req, res) => {
  ...
  if (!adminTelegramId || !isStaticAdminUser(adminTelegramId)) {
  ...
  try {
    ...
    const recentLogs = await getRecentLogsForAdminStats(firebaseDbUrl, 7, 50);
...
app.all("/api/admin-send-test", async (req, res) => {
  ...
  try {
    ...
    const telegramId = await getUserTelegramIdSimple(firebaseDbUrl, userId);

    if (!telegramId) {
      return res.status(404).json({ error: "User has no telegram ID registered" });
    }

    const success = await sendTelegramMessageSimple(botToken, telegramId, message);
    ...
    for (let i = 0; i < failed.length; i += BATCH_SIZE) {
      ...
      await Promise.all(
        batch.map(async (notification) => {
          try {
            const success = await sendTelegramMessageSimple(
              botToken,
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      for (let i = 0; i < players.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (player) => {
          try {
            const ok = await sendTelegramMessageSimple(botToken, player.telegramId, original.message);
            if (ok) sent++;
            else failed++;
          } catch {
            failed++;
